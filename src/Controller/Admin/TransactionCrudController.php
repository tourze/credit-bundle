...
#[AdminCrud(
    routePath: '/credit/transaction',
    routeName: 'credit_transaction'
)]
final class TransactionCrudController extends AbstractCrudController
{
    ...
    {
        return $crud
            ->setEntityLabelInSingular('积分交易')
            ->setEntityLabelInPlural('积分交易管理')
            ->setPageTitle(Crud::PAGE_INDEX, '积分交易列表')
            ->setPageTitle(Crud::PAGE_NEW, '新建积分交易')
            ->setPageTitle(Crud::PAGE_EDIT, '编辑积分交易')
            ->setPageTitle(Crud::PAGE_DETAIL, '积分交易详情')
            ->setDefaultSort(['createTime' => 'DESC'])
            ->setSearchFields(['eventNo', 'account.accountName', 'remark', 'relationId'])
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        yield TextField::new('eventNo', '事件编号')
            ->setColumns('col-md-6')
            ->setRequired(true)
            ->setMaxLength(50)
        ;
        ...
        yield MoneyField::new('amount', '变动金额')
            ->setColumns('col-md-4')
            ->setRequired(true)
            ->setCurrency('CNY')
    ...
    {
        return $filters
            ->add(TextFilter::new('eventNo', '事件编号'))
